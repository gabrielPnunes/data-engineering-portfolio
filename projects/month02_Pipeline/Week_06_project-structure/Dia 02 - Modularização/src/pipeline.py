...
def _load(dfs: dict, df: pd.DataFrame) -> None:
    for name, df_raw in dfs.items():
        load_layer(df_raw, str(PROCESSED_PATH.parent), "Data/Processed", f"raw_{name}")

    load_layer(df, str(PROCESSED_PATH.parent), "Data/Processed", "deliveries_transformed")
...
def run() -> None:
    ...
    try:
        ...
        validate_load(
            str(PROCESSED_PATH.parent / "Data" / "Processed" / "deliveries_transformed.parquet"),
